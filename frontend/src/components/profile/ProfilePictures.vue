<template>
      ...
      <div class="profile--pictures--container">
        ...
        <div class="row--pictures">
          ...
          >
            ...
            <input type="file" :ref="'fileInput' + (index + 3)" @change="onFileChange($event, index + 3)" style="display: none;">
          </div>
        </div>
      </div>
    </div>
  ...
  export default {
    // photos = [];
    // fetch('http://192.168.1.45:8081/getPhotos/' + localStorage.getItem('userName'), {
    //     method: 'GET',
    //     headers: {
    ...
    //         } else {
    //             alert('Erreur lors de la récupération des photos : ' + data.message);
    //         }
    //     })
    //     .catch((error) => {
          ...
          fetch('http://192.168.1.45:8081/updateUser', {
            ...
            .then((data) => {
              if (data.success) {
                ...
                // Mettre à jour l'image avec l'URL renvoyée par le serveur si nécessaire
                if (data.imageUrl) {
                  this.images[index] = data.imageUrl;
                }
              } else {
